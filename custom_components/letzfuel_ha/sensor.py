...
def _change_attrs(
    ...
    previous = coordinator.display_price(fp.previous) if fp.previous else None
...
def _tomorrow_attrs(
    ...
    if fp is None or fp.upcoming is None:
        return {}
    delta = coordinator.display_price(fp.upcoming) - coordinator.display_price(
        fp.current
...
def _refill_attrs(
    ...
    if coordinator.tank_size and level is not None:
        liters_needed = round(coordinator.tank_size * max(0.0, 1.0 - level / 100.0), 2)
    attrs: dict[str, Any] = {
        "current_level_pct": level,
        "tank_size": coordinator.tank_size,
        "liters_needed": liters_needed,
        "price_per_liter": _primary_price(coordinator),
        "level_source": coordinator.level_source,
    }
    if coordinator.level_source == LEVEL_SOURCE_ENTITY and coordinator.level_entity_id:
        attrs["level_entity_id"] = coordinator.level_entity_id
    return attrs
